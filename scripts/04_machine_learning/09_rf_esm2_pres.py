...
# fast lookup (replace slow loop)
cedar_idx = cedar.set_index("epitope_id")
label_lookup = cedar_idx.to_dict("index")


# valid id set per dataset (avoid recomputation inside loop)
...
def run_rf_analysis(df, name):
    ...
    #bos
    ("diff_bos", X_bos),
    ("diff_bos_pres", np.hstack([X_bos, X_pres])),
...
#save
pd.DataFrame(all_results).to_csv("esm2_rf_pres.csv", index=False)

with open("roc_esm2_rf_pres.pkl", "wb") as f:
